...
public class Day_12 {
	 ...
	 * @return
	 */
	private static long check2(long count, String curr, List<Integer> nums) {
		
		if (curr.charAt(0) == '.') {
			if (curr.length() == 1) {
				return count;
			} else {				
				return check2(count, curr.substring(1), new ArrayList<>(nums));
			}
		} else if (curr.charAt(0) == '?'){
			char[] next = curr.toCharArray();
			
			next[0] = '.';
			count = check2(count, new String(next), new ArrayList<>(nums));
				...
				if (nums.isEmpty()) {
					...
					if (curr.length() - 1 > len && curr.charAt(len) != '#') {
						if (curr.charAt(len) == '?') {
							char[] temp = curr.toCharArray();
							temp[len] = '.';
							return check2(count, new String(temp).substring(len), new ArrayList<>(nums));
						} else {							
							return check2(count, curr.substring(len), new ArrayList<>(nums));
						}
					} else {
						return count;
					}
